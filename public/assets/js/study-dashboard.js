(function (root) {
  ...
  const refresh = el('button', 'Odśwież'), more = el('button', 'Pokaż kolejne pule'); refresh.type = more.type = 'button'; more.hidden = true;
  const manager = el('a', 'Otwórz menadżer fiszek →', 'study-open-manager-btn'); manager.href = '/members/module/flashcards/';
  const heading = el('header', null, 'study-dashboard-heading'), tools = el('div', null, 'study-dashboard-tools');
  tools.append(manager, refresh); heading.append(el('h2', 'Nauka / Fiszki'), tools); host.append(heading, status, list, more);
  ...
  async function load(next = false) {
    ...
    try {
      ...
      decks = [...unique.values()]; cursor = data.cursor; render();
    } catch (_) { if (version === epoch) status.textContent = 'Nie udało się pobrać powtórek. Kliknij „Odśwież”.'; }
    finally { if (version === epoch) { busy = false; refresh.disabled = more.disabled = false; } }
  }
